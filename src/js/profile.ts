...
function updateProfile({name="Thistlebeard the Tipsy",email="@TipsyThistle",banner,avatar}:profileInfo){
...
async function fetchPosts(url:string) {
 console.log(localStorage.getItem("token"))
    const response = await fetch(url,{headers:{Authorization:`Bearer ${endpoint.getToken()}`}});
